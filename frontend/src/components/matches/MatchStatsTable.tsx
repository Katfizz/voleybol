...
export function MatchStatsTable({ stats, teamName }: MatchStatsTableProps) {
    if (stats.length === 0) {
        return <div className="text-center text-muted-foreground py-4">No hay estadísticas registradas para {teamName}.</div>;
    }

    return (
        <div className="border rounded-md">
            <div className="bg-muted/50 p-2 font-semibold text-center border-b">{teamName}</div>
            <Table>
                <TableHeader>
                    <TableRow>
                ...
                <TableBody>
                    {stats.map((stat) => (
                        <TableRow key={stat.id}>
                            ...
                            <TableCell className="text-center text-destructive">{stat.errors}</TableCell>
                        </TableRow>
                    ))}
                </TableBody>
            </Table>
