...
import { Trophy, Target, Shield, Zap, HandHelping, Activity, AlertTriangle, Eye } from "lucide-react";
                            ...
                            history.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="text-sm flex justify-between mb-2">
                                        <span className="text-xs">{item.match?.homeCategory?.name} vs {item.match?.awayCategory?.name}</span>
                                        <span className="font-mono font-bold text-xs">
                                            {item.match?.home_sets_won} - {item.match?.away_sets_won}
                                        </span>
                                    </div>
                                    <div className="text-xs text-muted-foreground grid grid-cols-4 gap-2 bg-muted/20 p-1 rounded">
                                        <span className="text-center">PTS: <span className="font-bold text-foreground">{item.points}</span></span>
                                        <span className="text-center">K: {item.kills}</span>
                                        <span className="text-center">BLK: {item.blocks}</span>
                                        <span className="text-center">ACE: {item.aces}</span>
                                    </div>
                                </div>
                            ))
                        ) : (
